...
def manejar_error_voz(error):
    '''Funcion para manejar errores de reconocimiento de voz'''
    if isinstance(error, sr.UnknownValueError):
        return "Lo siento, no entendí eso."
    elif isinstance(error, sr.RequestError):
        return "Lo siento, ha ocurrido un error en la conexión."
    else:
        return "Lo siento, ha ocurrido un error desconocido."
...
class Asistente():
    ...
    def procesar_comando(self, comando):
            ...
            if 'luna' in comando:
                ...
                self.abrir_mail()
                self.callado=True
            elif 'campus' in comando:
                self.abrir_campus()
                self.callado=True
            elif 'linkedin' in comando:
                self.abrir_linkedin()
                self.callado=True
    ...
    def abrir_campus(self):
        '''Funcion que abre el campus de la facultad'''
        url_campus = 'https://campusgrado.fi.uba.ar/my/'
        webbrowser.open(url_campus)
        self.speak('Abriendo el campus virtual...')
        
    def abrir_linkedin(self):
        '''Funcion que abre linkedin'''
        url_linkedin= 'https://www.linkedin.com/feed/'
        webbrowser.open(url_linkedin)
        self.speak('Abriendo Linkedin...')
